...
class Client extends CI_Controller
{
	...
	public function clientOnboarding()
	{
		...
		$data = [];
		$clients = [];
		$clients['crawlerinfo'] = $this->Clients->getUserInfo($id);
		$clients['allClinets'] = $this->Clients->getAllClient();
		$clients['crawlers'] = $this->Clients->getAllCrawler();
		$clients['teamMembers'] = $this->Clients->getAllTeamMember();
		$clients['timezones'] = $this->Crawler->getAllTimzones();
		...
		$data['full_content'] =  $this->load->view('client/client-onboarding', $clients, TRUE);
		$this->load->view('layout/master', $data);
	}
	...
	public function addclienttags()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$string = str_replace(' ', '-', $info['tag']); // Replaces all spaces with hyphens.
			$string = preg_replace('/[^A-Za-z0-9\-]/', '', $string); // Removes special chars.

			$getData = preg_replace('/-+/', '-', $string); // Replaces multiple hyphens with single one.

			$insertTag = [
				'crawler_id' => $this->session->userdata('id'),
				'tag' => $getData,
				'daily_report' => $info['daily_report'],
				'export_report' => $info['export_report'],
			];
			// echo "<pre>"; print_r($insertTag); die();

			$this->Clients->insertData('tags', $insertTag);

			$this->session->set_flashdata('sucess', 'Tag ' . $getData . ' has been added successfully.');
			redirect(base_url() . 'tags', 'refresh');
		}
	}
...
=========================================================================*/
	...
	public function getClientFilterData()
	{
		...
		if ($info['filter_tags'][0] == 0 || $info['filter_tags'][0] == 1 || $info['filter_tags'][0] == 2 || $info['filter_tags'][0] == 3) {
			...
			$config["total_rows"] = $this->Clients->count_allForCustom($clientId);
			...
			$page = ($this->uri->segment(2)) ? $this->uri->segment(2) : 0;
			$filterData["links"] = $this->pagination->create_links();
			$list = $this->Clients->get_datatablesCustom($config["per_page"], $page, $clientId);
			...
			foreach ($list as $recommendation) {
				...
				$row['fb_request_full_name'] = $recommendation->fb_request_full_name;
				$row['fb_request_content'] = $recommendation->fb_request_content;
				$row['keyword_id'] = $this->Clients->getkeyWord($recommendation->keyword_id);
				// echo $this->db->last_query();
				$row['fb_group_id'] = $this->Reports->getFacebookGroup($recommendation->fb_group_id);
				$row['source'] = $recommendation->source;
				$row['tags'] = $this->getAllTags($recommendation->tags);
...
=========================================================================*/
	...
	public function clientgroups()
	{
		...
		$user_data['usersinfo'] = $this->Clients->getUserInfo($id);
		// echo "<pre>"; print_r($user_data['crawlerinfo']); die();
		...
		if (count($info) > 0) {
			// echo "hi"; die();
			$user_data['usersinfo'] = $this->Clients->getUserInfo($id, $info['category_filter']);
		}

		$data['full_content'] =  $this->load->view('groups/index', $user_data, TRUE);
		$this->load->view('layout/master', $data);
	}
	...
	{
		$this->session->unset_userdata('keyword_data');
		$id = $this->session->userdata('id');
		$info = $this->input->post();
		// echo '<pre>'; print_r($info); die();

		$keywords = json_decode($info['keyword'], TRUE);
		foreach ($keywords as $keyword) {
			$getMulticrawlers = $this->Clients->getAllCrawlersUnderClient($id);
			$key = [];
			$key['keyword'] = $keyword['keyword'];
			$key['recommended_reply'] = $keyword['recommended_reply'];
			$key['must_include_keywords'] = $keyword['must_include_keywords'];
			$key['must_include_condition'] = $keyword['must_include_condition'];
			$key['must_exclude_keywords'] = $keyword['must_exclude_keywords'];
	...
	public function clientNotification(){
		...
		if(count($info) > 0){
			...
			for ($j = 0; $j < count($getMulticrawlers); $j++) {
				$insertNotification['crawler_id']  =  $getMulticrawlers[$j]['crawler_id'];
				$insertNotification['end_client_id'] = $getMulticrawlers[$j]['end_client_id'];
				// echo $this->db->last_query();
				// echo '<pre>'; print_r($insertNotification);
				$this->Crawler->insertData('notification_settings', $insertNotification);
			}
			$this->session->set_flashdata('success', 'Notification has been added successfully.');
		}
		$data['full_content'] =  $this->load->view('client/notification', $user_data, TRUE);
		$this->load->view('layout/master', $data);
	}
	...
	public function uploadKeywordCSv($id)
	{
		...
		if (isset($_FILES['enter_csv']['name'])) {
			...
			if ($this->upload->do_upload('enter_csv')) {
				...
				while ($row = fgetcsv($handle)) {
					if ($i != 1) {
						$keywordData[$i]['keyword'] = $row[0];
						$keywordData[$i]['must_include_keywords'] = $row[1];
						$keywordData[$i]['must_include_condition'] = $row[2];
						$keywordData[$i]['must_exclude_keywords'] = $row[3];
						$keywordData[$i]['recommended_reply'] = $row[4];
					}
					$i++;
				}
				
				$keywordData = array_values($keywordData);
				// echo "<pre>";  print_r($keywordData); die();

				$this->session->set_userdata('keyword_data', $keywordData);
				$this->session->set_flashdata('success', 'CSV uploaded successfully!');
			} else {
				$error = $this->upload->display_errors();
				$this->session->set_flashdata('csv_error', $error);
	...
	public function adminCategoryFilter()
	{
		$info = $this->input->post();
		$id = $info['user_id'];
		$sort = $info['cat_filter'];

		$results = $this->Clientonboardingmodel->adminCategoryFilter($id, $sort);
	...
	/*======================================
        All Data Client Profile
    ======================================*/
	public function clientOnboardingSubmit()
	{
		// echo "hi"; die();
		// echo "<pre>"; print_r($_POST); die();
		...
		$this->session->set_flashdata('success', 'Client onboarding from has been added successfully.');
		// die();
		redirect(base_url() . 'client_onboarding', 'refresh');
	}
}
